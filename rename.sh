...
hellper(){
        #statements
        echo "  使用格式："
        echo "         $0 文件路径 匹配  替换 替换模式 编号  繁简体转换 递归 "
        echo "  参数："
        echo "         文件路径：要重命名的文件所在目录"
        echo "         匹配：匹配要替换的字符串，该匹配为全局匹配（支持正则表达式）"
        echo "         替换：替换匹配的字符串，如果替换为空使用传入_del "
        echo "         替换模式：为y是替换文件，不传或为n是显示替换信息 "
        echo "         编号：如果编号传n，如果编号则传入开始数值！ "
        echo "         繁简体转换：为y是则繁体转换为简体，为n或不传是不转换！ "
        echo "         递归：如果为y则重命名子目录中的所有文件，为n或不传是不递归！ "

        echo "  示例："
        echo "         $0 /home/download \"[0-9]*\" \"aa\"  y 1 n n "
}
