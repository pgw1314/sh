...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/dht.dat"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			...
			if [[ ${sh_new_type} == "softs" ]]; then
				...
				wget -N --no-check-certificate https://raw.githubusercontent.com/pgw1314/sh/master/install/aria2/aria2.sh && chmod +x aria2.sh
			fi
